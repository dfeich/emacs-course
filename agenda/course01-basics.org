...
** TODO Buffers and switching between them [0/3]
   ...
   - [ ] You can switch to another buffer using the standard key-combo
     "C-c b" (switch-buffer). You will be prompted for the buffer's name.
...
** TODO Moving inside of a buffer
   ...
   - "M-g M-g" (goto-line) :: go to a specific line number
   - "M-g M-c" (goto-char) :: go to a specific character number within
     the file
